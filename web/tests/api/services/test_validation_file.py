...
def test_file_max_size(client, monkeypatch):
    monkeypatch.setitem(
        TextFileSerializer._declared_fields,
        "file",
        serializers.FileField(
            allow_empty_file=False,
            validators=[
                FileContentTypeValidator(
                    max_size=0.1 * 1024 * 1024, mime_types=TEXT_MIME_TYPES  # 0.1Mb
                )
            ],
        ),
    )
    ...
    with open(DOCX_FILE, "rb") as docx_file:
        response = client.post("/api/convert/text/to/pdf/", {"file": docx_file})
        assert response.status_code == 400
        assert response.json() == dict(
            file=[
                "Ensure this file size is not greater than 102.4"
...
def test_file_min_size(client, monkeypatch):
    monkeypatch.setitem(
        ...
        serializers.FileField(
            ...
            validators=[
                FileContentTypeValidator(
                    min_size=2 * 1024 * 1024, mime_types=TEXT_MIME_TYPES  # 2Mb
                )
            ],
        ),
    )
    ...
    with open(DOCX_FILE, "rb") as docx_file:
        ...
        assert response.json() == dict(
            file=[
                "Ensure this file size is not less than 2.0\xa0MBs. Your file size "
                "is 211.7\xa0KBs."
            ]
        )
...
def test_broken_file(client):
    with open(BROKEN_DOCX_FILE, "rb") as image_file:
        response = client.post("/api/convert/text/to/odt/", {"file": image_file})
    assert response.status_code == 400
...
def test_file_type(client):
    with open(INVALID_IMAGE_FILE, "rb") as image_file:
        response = client.post("/api/convert/text/to/pdf/", {"file": image_file})
    assert response.status_code == 400
    assert response.json() == dict(
        file=["Files of type image/pngs are not supported for this converter."]
    )
...
def test_valid_class_type(client):
    ...
    with tempfile.TemporaryDirectory(dir=os.path.join(settings.CONVERTER_TEMP_FOLDER)) as temp_dir:
        # unpack response file
        with tempfile.NamedTemporaryFile(dir=temp_dir, suffix=".zip") as temp_file:
        ...
        files = [
            ...
            if path.isfile(path.join(temp_dir, f)) and f != temp_file.name
        ]
