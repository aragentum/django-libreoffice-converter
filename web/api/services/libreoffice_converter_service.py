...
class LibreOfficeConverterService(Service):
    """
    Implements logic for running conversion process in LibreOffice via subprocess.
    """
    ...
    def convert_to(self, file_path: str, output_dir: str, converter: str):
        """
        Converts file by file_path in LibreOffice.
        """
        self.logger.debug(
            "Start converting file '%s' to %s in %s", file_path, converter, output_dir
        )
    ...
    def _parse_subprocess_stdout(self, process_stdout: bytes):
        """
        Parses result of subprocess from LibreOffice.
        """
        self.logger.debug("Parsing subprocess output '%s'", process_stdout)
        match_output = re.search(
            "-> (.*?) using filter", process_stdout.decode() if process_stdout else ""
        )
    ...
    def _run_libreoffice_subprocess(
        self, input_file_path: str, output_dir: str, converter: str
    ) -> bytes:
        """
        Runs converting process in subprocess using LibreOffice.
        """
        # prepare args
        ...
        # create subprocess
        process = subprocess.Popen(args, stdout=subprocess.PIPE, preexec_fn=os.setsid)
