...
class FileContentTypeValidator(object):
    ...
    def __call__(self, data):
        if self.max_size is not None and data.size > self.max_size:
            raise ValidationError(self.error_messages['max_size'].format(
                ...
                size=filesizeformat(data.size)), 'min_size')

        if self.min_size is not None and data.size < self.min_size:
            raise ValidationError(self.error_messages['min_size'].format(
        ...
        if self.mime_types:
            ...
            if extensions is None:
                raise ValidationError(self.error_messages['mime_type'].format(
    ...
    def __eq__(self, other):
        return (
                isinstance(other, FileContentTypeValidator) and
